class EventDecorator < ApplicationDecorator
  ...
  def attending
    attendances.joins(:invitee).attending.order.(:created_at).order('invitees.party_id')
  end

  def declined
    attendances.joins(:invitee).declined.order(:created_at).order('invitees.party_id')
  end
